...
namespace evpp {
    ...
    void Connector::Start() {
        ...
        timer_.reset(
                new TimerEventWatcher(loop_, std::bind(&Connector::OnConnectTimeout, shared_from_this()), timeout_));
        timer_->Init();
        timer_->AsyncWait();
    ...
    void Connector::HandleWrite() {
        ...
        if (status_ == kDisconnected) {
            ...
            LOG_INFO << "fd=" << chan_->fd() << " remote_addr=" << remote_addr_
                     << " receive write event when socket is closed";
            return;
        }
    ...
    void Connector::HandleError() {
        ...
        LOG_ERROR << "this=" << this << " status=" << StatusToString() << " fd=" << fd_ << " use_count="
                  << self.use_count() << " errno=" << serrno << " " << strerror(serrno);
        ...
        if (owner_tcp_client_->auto_reconnect()) {
            ...
            DLOG_TRACE << "loop=" << loop_ << " auto reconnect in " << owner_tcp_client_->reconnect_interval().Seconds()
    ...
    void Connector::OnConnectTimeout() {
        LOG_WARN << "this=" << this << " Connector::OnConnectTimeout status=" << StatusToString() << " fd=" << fd_
                 << " this=" << this;

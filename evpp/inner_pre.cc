...
namespace evpp {
    ...
    int EventAdd(struct event *ev, const struct timeval *timeout) {
        ...
        LOG_DEBUG << "event_add ev=" << ev << " fd=" << ev->ev_fd << " user_ptr=" << ev->ev_arg << " tid="
    ...
    int EventDel(struct event *ev) {
#ifdef H_DEBUG_MODE
        {
            ...
            if (it == evmap.end()) {
                LOG_ERROR << "Event " << ev << " fd=" << ev->ev_fd
                          << " not exist in event loop, maybe event_del twice.";
        ...
        LOG_DEBUG << "event_del ev=" << ev << " fd=" << ev->ev_fd << " user_ptr=" << ev->ev_arg << " tid="
                  << std::this_thread::get_id();
#endif
        return event_del(ev);
    }

...
namespace evpp {
    ...
    void EventLoop::QueueInLoop(const Functor &cb) {
        DLOG_TRACE << "pending_functor_count_=" << pending_functor_count_ << " PendingQueueSize="
                   << GetPendingQueueSize() << " notified_=" << notified_.load();
        {
#ifdef H_HAVE_BOOST
        ...
        DLOG_TRACE << "queued a new Functor. pending_functor_count_=" << pending_functor_count_ << " PendingQueueSize="
                   << GetPendingQueueSize() << " notified_=" << notified_.load();
        if (!notified_.load()) {
    ...
    void EventLoop::QueueInLoop(Functor &&cb) {
        ...
        if (!notified_.load()) {
            ...
            if (watcher_) {
                ...
                DLOG_TRACE << "watcher_ is empty, maybe we call EventLoop::QueueInLoop on a stopped EventLoop. status="
                           << StatusToString();
                assert(!IsRunning());
            }
        } else {

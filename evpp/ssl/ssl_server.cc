...
namespace evpp {
    namespace ssl {
        ...
        bool SSLServer::Start() {
            ...
            if (rc) {
                ...
                listener_->SetNewConnectionCallback(
                        std::bind(&SSLServer::HandleNewConn,
                                  this,
                                  std::placeholders::_1,
                                  std::placeholders::_2,
                                  std::placeholders::_3));
        ...
        void SSLServer::Stop(DoneCallback on_stopped_cb) {
            DLOG_TRACE << "Entering ...";
            assert(status_ == kRunning);
            status_.store(kStopping);
            substatus_.store(kStoppingListener);
            loop_->RunInLoop(std::bind(&SSLServer::StopInLoop, this, on_stopped_cb));
        }

        void SSLServer::StopInLoop(DoneCallback on_stopped_cb) {
            DLOG_TRACE << "Entering ...";
            ...
            if (connections_.empty()) {
                ...
                for (auto &c : connections_) {
                    if (c.second->IsConnected()) {
                        ...
                        DLOG_TRACE
                                << "Do not need to call Close for this TCPConn it may be doing disconnecting. TCPConn="
                                << c.second.get() << " fd=" << c.second->fd() << " status=" << StatusToString();
                    }
                }
        ...
        void SSLServer::HandleNewConn(evpp_socket_t sockfd,
            ...
            TCPConnPtr conn(new TCPConn(io_loop, n, sockfd, listen_addr_, remote_addr, next_conn_id_, ssl_ctx_));
        ...
        EventLoop *SSLServer::GetNextLoop(const struct sockaddr_in *raddr) {
            if (IsRoundRobin()) {
        ...
        void SSLServer::RemoveConnection(const TCPConnPtr &conn) {
            DLOG_TRACE << "conn=" << conn.get() << " fd=" << conn->fd() << " connections_.size()="
                       << connections_.size();
            auto f = [this, conn]() {

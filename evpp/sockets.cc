...
namespace evpp {
    ...
    namespace sock {
        ...
        bool ParseFromIPPort(const char *address, struct sockaddr_storage &ss) {
            ...
            if (rc == 0) {
                LOG_INFO << "ParseFromIPPort evutil_inet_pton (AF_INET '" << host.data() << "', ...) rc=0. "
                         << host.data() << " is not a valid IP address. Maybe it is a hostname.";
                return false;
            } else if (rc < 0) {
                int serrno = errno;
                if (serrno == 0) {
                    ...
                    LOG_WARN << "ParseFromIPPort evutil_inet_pton (AF_INET, '" << host.data() << "', ...) failed : "
                             << strerror(serrno);
                }
                return false;
            }
